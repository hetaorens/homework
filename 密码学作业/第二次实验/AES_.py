...
class T_demo_aes:
    ...
    def __init__(self,key,plaintext):
        ...
        self.key=key
    def aes_vs_myaes(self):
        aes = MY_AES()
        RoundKeys = aes.round_key_generator(key)

        plaintext_ = aes.num_2_16bytes(plaintext)
        ciphertext = aes.aes_encrypt(plaintext_, RoundKeys)
        print("MY AES is as follows")
        print('ciphertext = ' + hex(aes._16bytes2num(ciphertext)))
        ciphertext=aes._16bytes2num(ciphertext)
        # ciphertext = 0x69c4e0d86a7b0430d8cdb78070b4c55a
        _ciphertext = aes.num_2_16bytes(ciphertext)
        _plaintext = aes.aes_decrypt(_ciphertext, RoundKeys)
        print('plaintext = ' + hex(aes._16bytes2num(_plaintext)))
        print("Standard AES is as follows")
        aes_ = AES.new(key=long_to_bytes(key), mode=AES.MODE_CBC, IV=bytes(16))
        # C = "0x" + aes_.encrypt(long_to_bytes(plaintext)).hex()
        C=aes_.encrypt(long_to_bytes(plaintext))
        aes_ = AES.new(key=long_to_bytes(key), mode=AES.MODE_CBC, IV=bytes(16))
        print('ciphertext = ' +"0x" +C.hex())
        P =aes_.decrypt(C)
        print('plaintext = '+"0x"+P.hex())

if __name__ == '__main__':
    plaintext = 0x3243f6a8885a308d313198a2e0370734
    key = 0x2b7e151628aed2a6abf7158809cf4f3c
    test=T_demo_aes(key,plaintext)
    test.aes_vs_myaes()
    print("one byte changed")
    plaintext = 0x3243f6a8885a308d313198a2e0370734
    key = 0x2b7e151628aed2a6abf7158809cf4f31
    test = T_demo_aes(key, plaintext)
    test.aes_vs_myaes()
